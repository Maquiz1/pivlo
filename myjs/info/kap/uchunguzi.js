for (let i = 1; i <= 100; i++) {
  const ever_smoked = `ever_smoked_${i}`;
  const smoking = `smoking_${i}`;

  const dropdown = document.getElementById(ever_smoked);
  const elementToShow = document.getElementById(smoking);
